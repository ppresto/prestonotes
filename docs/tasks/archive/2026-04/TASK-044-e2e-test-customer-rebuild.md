...
**Archived 2026-04-24:** Operator closed this task **outside the file** — the **canonical** harness is the **eight-step** flow in [`docs/ai/playbooks/tester-e2e-ucn.md`](../../ai/playbooks/tester-e2e-ucn.md) + [`scripts/e2e-test-customer.sh`](../../../scripts/e2e-test-customer.sh) + **TASK-052** history. This document remains as **background / intent**; do not treat incomplete checklists below as blocking.

**Status (historical):** was ON HOLD — see archive note above.
**Opened:** 2026-04-21
...
**Canonical E2E context (current state of the world):** [`.cursor/agents/tester.md`](../../../.cursor/agents/tester.md) and [`docs/ai/playbooks/tester-e2e-ucn.md`](../ai/playbooks/tester-e2e-ucn.md) (eight steps). **Default harness + Drive** shipped under **[TASK-052 (archive)](../archive/2026-04/TASK-052-e2e-test-customer-drive-sync-and-artifact-survival.md)**; this file’s “10 steps / delete-first” spec is **superseded in practice** by the eight-step playbook + `prep-v1` / `prep-v2` until this task is un‑hold and reconciled.

## Deferral note (2026-04-22)

This task remains **valid** and **incomplete** (acceptance checklists below are still authoritative). It is **on hold** while **TASK-053** (UCN/GDoc quality TOC) proceeds; see [`docs/tasks/INDEX.md`](../INDEX.md). **TASK-051** / **TASK-052** are **archived** (harness + call-record schema shipped). Resume **TASK-044** when you want a final **delete** of duplicate harness scripts and a single entry point, after the guide + **TASK-053** stabilize.
...
- **Playbook:** `docs/ai/playbooks/tester-e2e-ucn.md` — the only doc the agent needs to follow.
...
#### B2. `docs/ai/playbooks/tester-e2e-ucn.md`
...
- [ ] `docs/ai/playbooks/tester-e2e-ucn.md` lists the canonical 10-step sequence and nothing else.
...
- Rewritten: `docs/ai/playbooks/tester-e2e-ucn.md` — canonical 10-step sequence, contract, prerequisites, manual verification checklist.
