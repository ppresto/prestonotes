...
**Pass criteria (result reporting):** For the default `e2e_default` run, a **`success`** outcome **requires** completing **all** harness steps **1 through 7** in [`scripts/lib/e2e-catalog.txt`](../../scripts/lib/e2e-catalog.txt) (both prep shell steps, both UCNs, and both extract + load blocks as defined there). A run that stops after extract or after prep-v1 only (or that never performs the real UCN writes) is **`failed`**, not **`success`** and not a soft “blocked, resume later” — see **`.cursor/agents/tester.md` — Completion and status semantics** and **`.cursor/rules/11-e2e-test-customer-trigger.mdc` — Harness result**).

**Where to run (supported vs experimental):** For a **full** 1–7 E2E you need to pass, use the **primary (main) Cursor chat** and keep the harness the **active** task **until** it finishes (foreground in that chat). Running the **entire** 1–7 flow as a **background** `/tester` subagent is **not** supported for a trustworthy pass today (turn limits and early handoff risk); if you do try it, treat the outcome as **experimental** and any early stop as **`failed`**, not `success`. Future automation (scripted or split delegations) will make background orchestration viable.
...
- **Artifact hygiene:** customer-facing artifacts produced by this flow (GDoc sections, History Ledger, `challenge-lifecycle.json`, call records) must be indistinguishable from a real-customer run. See **`.cursor/rules/11-e2e-test-customer-trigger.mdc` — Artifact hygiene**.
...
- **MCP servers enabled in Cursor (fail-fast before the harness):** Chat steps **2–4** and **6–7** require the **prestonotes** MCP server; Wiz product-doc adjacency expects **wiz-remote** (see **`.cursor/mcp.json`** keys). The **`/tester`** agent runs **read-only smokes** in **Session init** (`.cursor/agents/tester.md`) — **`check_google_auth`** on **prestonotes** and **`wiz_docs_knowledge_base`** on **wiz-remote** — and returns **`blocked`** if either is down, so you do not get a shell-only “green” with MCP missing.
...
Use the trigger **Run E2E Test Customer** (or the phrases in `.cursor/rules/11-e2e-test-customer-trigger.mdc`). The agent runs the shell and chat order from **`list-steps` / the catalog** (shell steps **1** and **5**, chat **2–4** and **6–7** in current defaults) with the `_TEST_CUSTOMER` approval bypass so there are **no** write-approval stops for that customer.
...
2. **Shell steps** (1 and 5 only in current catalog):  
...
- `harness-steps-1-7.checklist.md`
