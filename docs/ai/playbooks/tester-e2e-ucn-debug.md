...
**Purpose:** A **live Cursor session** with an agent that runs the `_TEST_CUSTOMER` pipeline **in stages**, **pauses for your approval** between stages, and **investigates gaps** between what the **transcripts** (and optional **call records** if you ran Extract) contain vs what landed in **`_TEST_CUSTOMER Notes`** (Google Doc) and in on-disk artifacts. This is **not** a lint script; the agent uses MCP, `gcloud`/Drive API as needed, file reads, tests, and code tracing.
...
## Phase A — First UCN (`prep-v1` → context → UCN) then **pause and diff**

**Goal:** Establish what the **first** UCN run wrote vs what **should** have been representable from v1 transcripts (and optional call records if present).

### A1. Run the pipeline to first UCN (stop before `prep-v2`)

Execute **in order**, in this session (same triggers as the main E2E playbook’s steps **1–3**):
...
3. **Chat** — *Update Customer Notes for _TEST_CUSTOMER* (first UCN; per [`update-customer-notes.md`](update-customer-notes.md)).

**Optional (extract-only debugging):** If you are isolating **Extract** quality, run *Extract Call Records for _TEST_CUSTOMER* per [`extract-call-records.md`](extract-call-records.md) **before** step 3 and run `uv run python -m prestonotes_mcp.call_records lint _TEST_CUSTOMER` until it exits 0. This is **not** part of the default unattended E2E harness.
...
> *Approved. Continue to Phase B (prep-v2, second UCN, lifecycle + metadata check).*

If the operator is still iterating on Phase A, stay in A3/A4.

---

## Phase B — `prep-v2` → second UCN, then **diff + lifecycle + metadata**
...
1. **Shell** — `./scripts/e2e-test-customer.sh prep-v2` (requires round-1 UCN to have completed so local/Drive state matches your expectations before v2 transcript merge).
2. **Chat** — *Update Customer Notes for _TEST_CUSTOMER* (second UCN).

**Optional:** Second *Extract Call Records for _TEST_CUSTOMER* + lint before step 2 when debugging call-record coverage for v2 transcripts.
...
- *Phase B: continue E2E troubleshoot — prep-v2, second UCN, lifecycle and metadata diff per `tester-e2e-ucn-debug.md`*
