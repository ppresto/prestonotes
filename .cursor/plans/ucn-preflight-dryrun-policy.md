...
**E2E vs UCN:** The E2E doc is a **procedure** for `_TEST_CUSTOMER` (prep shell + UCN + diff). It should **not** re‑own general UCN policy; it should **point** at the UCN playbook for “how to UCN” and add only what is **specific to the harness** (e.g. approval bypass, optional writer dry-run in test).
...
| **Writer `dry_run` as an optional E2E step** | **[`tester-e2e-ucn.md`](docs/ai/playbooks/tester-e2e-ucn.md)** only — under the UCN steps (3 and 5) or a short “UCN validation” note. | **`11-e2e` / `tester.md` / `e2e-catalog`:** at most **one** pointer: “E2E optional writer dry-run: see `tester-e2e-ucn.md`.” **Do not** document production “no dry-run” in E2E-only files as a second SSoT; that stays in the UCN playbook. |
...
2. [`docs/ai/playbooks/tester-e2e-ucn.md`](docs/ai/playbooks/tester-e2e-ucn.md) — add **only** the **optional** `update-gdoc-customer-notes.py write --dry-run` (or MCP `write_doc` `dry_run=true`) for the **`_TEST_CUSTOMER`** harness, after preflight; link to the UCN playbook for preflight rules.
...
| [`.cursor/rules/20-orchestrator.mdc`](.cursor/rules/20-orchestrator.mdc) | Block B: “`write_doc` `dry_run=true` preview when available” | **Replace** with: before real write, run **`ucn-planner-preflight`** per UCN playbook; **writer** dry-run only where [`tester-e2e-ucn.md`](../docs/ai/playbooks/tester-e2e-ucn.md) says (E2E). |
...
| [`docs/ai/playbooks/update-customer-notes.md`](docs/ai/playbooks/update-customer-notes.md) | v2 blurb + Step 10 still say `dry_run=true` for every Cursor `write_doc` | **Edit in place** (SSoT): preflight required; align `dry_run` with **E2E-only** policy in same doc. |
...
| Running the **`_TEST_CUSTOMER` E2E harness** (prep + UCN + diff) | [`tester-e2e-ucn.md`](docs/ai/playbooks/tester-e2e-ucn.md) | E2E-only: shell steps, approval bypass, optional **writer** dry-run. **Also** link UCN for “how to UCN” content. |
