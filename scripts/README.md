...
- **E2E `_TEST_CUSTOMER` harness:** canonical procedure **`docs/ai/playbooks/tester-e2e-ucn.md`**; entry script **`./scripts/e2e-test-customer.sh`** (`prep-v1` / `prep-v2` / `reset` / `list-steps`). Background/history: **`docs/tasks/archive/2026-04/TASK-044-e2e-test-customer-rebuild.md`** (archived). The old overlapping scripts (`e2e-test-customer-prep.sh`, `e2e-reset-test-customer-drive.sh`, `e2e-full-validation.*`, `e2e-test-customer-report.py`, `e2e-test-customer-verify.py`, `refresh_test_customer.py`, `seed-test-customer-challenge-lifecycle.py`, …) have been removed.
